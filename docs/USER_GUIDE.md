...
- **Role-based access** — two Supabase Auth accounts gate the dashboard. The couple account (`VITE_COUPLE_EMAIL`) gets full access. The bridal team account (`VITE_HELPER_EMAIL`) is locked to D-Day controls only: guest check-in, angbao recording, lucky draw, and a read-only seating chart. Planning tabs, guest add/delete/edit, exports, and financial totals are hidden. **This split is a client-side UX gate, not a server-enforced security boundary:** after Supabase authenticates the shared account, the role is computed in the browser from the signed-in email (`getRole()` in `src/lib/supabase.js`) and used only to hide couple-only controls. Data-level access is governed by RLS, which currently grants any authenticated user full CRUD and does **not** distinguish couple from helper — so a helper (or anyone with the helper credentials) can still reach the hidden data directly via the API/SDK. Treat the couple/helper split as a convenience, not a guarantee (see **Residual risk** below).
