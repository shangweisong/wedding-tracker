-- 0008_hero_focal_point.sql — hero image focal point for off-centre photos (#75)
-- Adds a `hero_focal_point` column holding a `background-position` preset (one of
-- a 9-point grid, default 'center') and threads it through the read/write RPCs.
-- The public hero keeps `background-size: cover`; this only re-anchors the crop so
-- an off-centre couple isn't cropped out. Idempotent & consolidated, consistent
-- with migrations 0001–0007.

-- ── 1. Column ─────────────────────────────────────────────────────────────────
alter table public.weddings
  add column if not exists hero_focal_point text not null default 'center';

-- Whitelist guard: upsert_wedding_page is granted to anon, so a caller can bypass
-- the client-side normalizeFocalPoint() whitelist. Bound the stored value to the 9
-- valid background-position presets so only safe CSS is ever served on the public
-- page. (Postgres has no `add constraint if not exists`, so guard on pg_constraint
-- for idempotency, mirroring the section_photos size guard in 0007.)
do $$
begin
  if not exists (
...
-- ── 2. RPCs (recreate with hero_focal_point) ──────────────────────────────────
...
-- 2b. Admin write (Wedding Page tab). Adds p_hero_focal_point.
...
grant execute on function public.upsert_wedding_page(text, text, text, text, jsonb, date, boolean, text, text, text, boolean, text, text, jsonb, jsonb, jsonb, text)
