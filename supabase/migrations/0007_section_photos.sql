-- 0007_section_photos.sql — optional photo galleries between page sections (#71)
-- Adds a `section_photos` JSONB column holding, per insertion slot, an
-- { enabled, cols, photos[] } record, and threads it through the read/write
-- RPCs. Rendered on the public page between content sections when enabled.
-- Idempotent & consolidated, consistent with migrations 0001–0006.
...
-- 2b. Admin write (Wedding Page tab). Adds p_section_photos.
...
grant execute on function public.upsert_wedding_page(text, text, text, text, jsonb, date, boolean, text, text, text, boolean, text, text, jsonb, jsonb, jsonb)
